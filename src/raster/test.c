...
#define SCREENSHOT_2026_03_24_144117_HEIGHT 160
#define SCREENSHOT_2026_03_24_144117_WIDTH 248
...
void print_splash(UINT8 *base)
{
    ...
    INT16 label_col;
    INT16 top_label_row;
    INT16 bottom_label_row;
    UINT16 sprite_height = SCREENSHOT_2026_03_24_144117_HEIGHT;
    UINT16 sprite_width = SCREENSHOT_2026_03_24_144117_WIDTH;
    UINT16 row_bytes = (sprite_width + 7) / 8;
    UINT16 page_bytes = (sprite_height + 7) / 8;
    UINT16 chunk_count = (sprite_width + 31) / 32;
    UINT16 row_major_bytes = (UINT16)(row_bytes * sprite_height);
    UINT16 page_packed_bytes = (UINT16)(sprite_width * page_bytes);
    UINT16 sprite_bytes = (UINT16)sizeof(SPLASH_MAP);
    int use_page_packed = (sprite_bytes == page_packed_bytes);
    UINT8 chunk_buffer[SCREENSHOT_2026_03_24_144117_HEIGHT * 4];
    INT16 start_row = (INT16)((400 - sprite_height) / 2);
    INT16 start_col = (INT16)((640 - sprite_width) / 5);
    INT16 i;

    rect_col = (INT16)(start_col + sprite_width);
    rect_top = (INT16)(start_row + ((sprite_height - 58)) / 4);
    label_col = (INT16)(rect_col + 20);
    top_label_row = (INT16)(rect_top + 8);
    bottom_label_row = (INT16)(rect_top + 80 + 8);

    fill_screen((UINT32 *)base, -1); /* fill with black */

    /* Draw 64-pixel grid pattern with white lines (color 0) */
    /* Draw horizontal lines */
    for (i = 0; i < 400; i += 16)
    {
        plot_line((UINT32 *)base, i, 0, i, 639, 0);
    }

    /* Draw vertical lines */
    for (i = 0; i < 640; i += 16)
    {
        plot_line((UINT32 *)base, 0, i, 399, i, 0);
    }

    for (chunk = 0; chunk < chunk_count; chunk++)
    {
        UINT16 dst_col = (UINT16)(chunk * 32);
        UINT16 remaining_pixels = sprite_width - (chunk * 32);
        ...
        for (row = 0; row < sprite_height; row++)
        {
            ...
            for (bit = 0; bit < remaining_pixels; bit++)
            {
                ...
                if (!use_page_packed && sprite_bytes >= row_major_bytes)
                {
                    src_index = (UINT16)(row * row_bytes + (x >> 3));
                ...
                src_byte = (src_index < sprite_bytes) ? SPLASH_MAP[src_index] : 0;
                if ((src_byte & src_mask) != 0)
                    dst_row[bit >> 3] |= (UINT8)(0x80 >> (bit & 7));
            }
        }

        plot_bitmap_32(base, start_row, (INT16)(start_col + dst_col), (const UINT32 *)chunk_buffer, sprite_height);
    }

    /* Two stacked 64x32 overlays, 32px in from the bitmap's right edge. */
    clear_region((UINT32 *)base, rect_top, rect_col, 32, 64);
    clear_region((UINT32 *)base, (INT16)(rect_top + 80), rect_col, 32, 64);
    plot_string(base, top_label_row, label_col, "Play");
    plot_string(base, bottom_label_row, label_col, "exit");

    /* Corner frame accents with left/right swapped orientation. */
    plot_triangle((UINT32 *)base, 0, 0, 56, 56, 1);
    plot_triangle((UINT32 *)base, 0, 639, 56, 56, 0);
    plot_triangle((UINT32 *)base, 399, 0, 56, 56, 3);
...
int main(void)
{
    ...
    if (cursor == PLAY)
        plot_string(base, 350, 280, "Play selected!");
    else
        plot_string(base, 350, 280, "Exit selected!");
